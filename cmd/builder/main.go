...
import (
	...
	"github.com/kaito-project/kaito/pkg/builder/controllers"
	"github.com/kaito-project/kaito/pkg/builder/webhooks"
...
func main() {
	...
	flag.StringVar(&featureGates, "feature-gates", "", "Enable Kaito feature gates.")
	opts := zap.Options{
		Development: true,
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "builder.kaito.sh",
	...
	builderReconciler := controllers.NewBuilderReconciler(
		kClient,
		mgr.GetScheme(),
		log.Log.WithName("controllers").WithName("Builder"),
		mgr.GetEventRecorderFor("KAITO-Builder-controller"),
	)

	if err = builderReconciler.SetupWithManager(mgr); err != nil {
		klog.ErrorS(err, "unable to create controller", "controller", "Builder")
		exitWithErrorFunc()
	}
	...
	if enableWebhook {
		...
		ctx := webhook.WithOptions(ctx, webhook.Options{
			...
			Port:        p,
			SecretName:  "builder-webhook-cert",
		})
		ctx = sharedmain.WithHealthProbesDisabled(ctx)
		ctx = sharedmain.WithHADisabled(ctx)
		go sharedmain.MainWithConfig(ctx, "webhook", ctrl.GetConfigOrDie(), webhooks.NewBuilderWebhooks()...)
